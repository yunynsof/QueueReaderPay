/**
 * ReaderQueuePayMasterThread.java
 ...
 * @version  1.0.0
 * @since 08-25-2022 09:19:15 PM 2022
 */
public class ReaderQueuePayMasterThread extends Thread {

	/** The Constant LOGGER. */
	private static final Logger LOGGER = LogManager.getLogger(ReaderQueuePayMasterThread.class);
	...
	/**
	 * Instantiates a new reader queue pay master thread.
	 */
	public ReaderQueuePayMasterThread() {
		try {
			initialize();
		} catch (Exception e) {
	...
	private boolean startupQueueConnection() {
		...
		properties.put("java.naming.provider.url", params.get("T3_QUEUE_READER_PAY"));
		try {
			ctx = new InitialContext(properties);
			qcf = (QueueConnectionFactory) ctx.lookup(params.get("CONNECTION_QUEUE_CF_QRP"));
			this.queue = (Queue) ctx.lookup(params.get("CONNECTION_QUEUE_DQ_QRP"));
	...
	public void run() {
		...
		while (state == States.STARTED) {
			...
			try {
				...
				NewRelicImpl.addNewRelicMetric("QueueReaderPay", timeDuration); // Se manda la informacion de
		...
		if (mensaje != null && !mensaje.equals("")) {
			...
			if (response != null) {
				...
				if (response.getCode() != 0) {
					NewRelicImpl.addNewRelicErrorMessage("QueueReaderPay " + response.getMessage());
				}
			}
		}
		return startTime;
